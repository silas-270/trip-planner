...
export async function updateWorkspace(body) {
    const response = await apiFetch(`${API_URL}/workspaces/${body.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(body),
    })
...
export async function requestAccess(code) {
    const response = await apiFetch(`${API_URL}/invite/access`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
// Create Share Code
export async function createAccess(body) {
    const response = await apiFetch(`${API_URL}/invite/create`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(body),
    })
...
// Exit Workspace
export async function delteAccess(body) {
    const response = await apiFetch(`${API_URL}/invite/leave`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(body),
    })
...
export async function getCards(workspaceId) {
    const response = await apiFetch(`${API_URL}/workspaces/${workspaceId}/cards`)
    return await response.json()
}
...
export async function deleteCard(workspaceId, cardId) {
    const response = await apiFetch(`${API_URL}/workspaces/${workspaceId}/cards/${cardId}`, {
        method: 'DELETE',
    })
}
...
export async function submitVote(cardId, vote) {
    ...
    try {
        const response = await apiFetch(`${API_URL}/vote`, {
            method: 'POST',
            headers: {
...
export async function fetchImages(query, index = 1) {
    ...
    try {
        const url = `${API_URL}/images?q=${encodeURIComponent(query)}&index=${index}`
