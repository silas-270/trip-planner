...
import { AddButton, SvgButton, Popup } from '../uielements/uielements';
import { IconT, IconBooking, IconImmoScout, BackArrow, Share, Edit, MenuDots } from '../../assets/svg';
        ...
        <div>
            <div className={styles.sectionWrapper}>
                <div className={styles.leftGroup}>
                    <SvgButton svg={<BackArrow />} onClick={backToSpaces} />
                    <SvgButton svg={<MenuDots />} onClick={() => setShowWorkspaceMenu(prevState => !prevState)} />
                    {showWorkspaceMenu &&
                        <>
                            <SvgButton svg={<Edit />} onClick={() => console.log('will add later')} />
                            <SvgButton svg={<Share />} onClick={() => someFunction} />
                        </>
                    }
                    <h2 className={styles.sectionHeading}>{wsMeta?.name?.trim() || 'Workspace'}</h2>
                </div>
                <AddButton mobileText='➕' desktopText='➕ Unterkunft hinzufügen' onClick={() => setShowPopup(true)} />
            </div>
